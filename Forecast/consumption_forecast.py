...
df_init['day_of_week'] = df_init[['day_of_week']].replace(["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],
                                                      [0,1,2,3,4,5,6])
#remove weekend
...
def forecasthour():
  ...
  denorm = InpData[0,:,0] * (N.max(axis=0)- N.min(axis=0)) + N.min(axis=0)
  ...
  #criar a tabela
  forecastdb = db1.forecastvaluerightside

  #inserir objeto em forma de dicionario em mongodb
  forecastdb.insert_one({"forecast_power": str(Pred.iat[0,0]),
              "datetime" : datetime.datetime.now() + datetime.timedelta(minutes=45)})
...
def forecastday():
  ...
  db1 = client.ForecastDay
        
  #criar a tabela
  forecastdb = db1.forecastvalue

  #inserir objeto em forma de dicionario em mongodb
  forecastdb.insert_one({"forecast_power": {"0": str(Pred.iat[0,0]),
                                            "1": str(Pred.iat[1,0]),
                                            "2": str(Pred.iat[2,0]),
                                            "3": str(Pred.iat[3,0]),
                                            "4": str(Pred.iat[4,0]),
                                            ...
                                            "8": str(Pred.iat[8,0]),
                                            "9": str(Pred.iat[9,0]),
                                            "10": str(Pred.iat[10,0]),
                                            "11": str(Pred.iat[11,0]),
                                            "12": str(Pred.iat[12,0]),
                                            "13": str(Pred.iat[13,0]),
                                            "14": str(Pred.iat[14,0]),
                                            "15": str(Pred.iat[15,0]),
                                            "16": str(Pred.iat[16,0]),
                                            "17": str(Pred.iat[17,0]),
                                            "18": str(Pred.iat[18,0]),
                                            "19": str(Pred.iat[19,0]),
                                            "20": str(Pred.iat[20,0]),
                                            "21": str(Pred.iat[21,0]),
                                            "22": str(Pred.iat[22,0]),
                                            "23": str(Pred.iat[23,0])},
              "datetime" : datetime.datetime.now() + datetime.timedelta(minutes=45)})
